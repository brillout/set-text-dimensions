(function(){
    ...
    window.set_text_dimensions = function(element, width_target, height_target){

        var clone = hidden_clone.generate(element);

        //hide_flickering.activate(clone);

        typeof assert !== "undefined" && assert(clone.textContent, "set_text_dimensions: element has no text: "+element);
    ...
    function refine(element, width_target, height_target) { 
        ...
        while( max_iter-- ) {
            ...
            if( number_of_no_resize === 10 ) {
                typeof assert !== "undefined" && assert(false, 'set_text_dimensions: incrementing/decrementing font-size does not change size of element');
                return;
            }
        ...
        typeof assert !== "undefined" && assert(false, 'set_text_dimensions: maximum number of iterations has been reached to compute font size');
    } 
